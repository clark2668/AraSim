...
//        cout<<"total station_count : "<<station_count<<endl;
	...
	if (settings1->ANTENNA_MODE == 0){
	  ...
	  ReadVgain("Vpol_original_CrossFeed_150mmHole_Ice_ARASim.txt");
	  // test read H-pol gain file!!
	  ReadHgain("Hpol_original_150mmHole_Ice_ARASim.txt");
	}
	else if (settings1->ANTENNA_MODE == 1) {
	  // test read V-pol gain file!!
	  ReadVgainSettings("Vpol_original_CrossFeed_150mmHole_Ice_ARASim.txt", settings1);
	  ReadVgainTopSettings("ARA_VPresult_topTrec.txt", settings1);
	  // test read H-pol gain file!!
	  ReadHgainSettings("Hpol_original_150mmHole_Ice_ARASim.txt", settings1);
	}
        ...
        while (NecOut.good() ) {
            ...
            for (int i=0; i<freq_step; i++) {
                ...
                if ( line.substr(0, line.find_first_of(":")) == "freq ") {
                    ...
                    //                    cout<<"freq["<<i<<"] = "<<Freq[i]<<" MHz"<<endl;
                    getline (NecOut, line); //read SWR
           		     Transm[i] = atof(line.substr(5,11).c_str()); //What says "SWR" in "Vpol_original_CrossFeed_150mmHole_Ice_ARASim.txt" is actually trasnmission coefficient - MYL 01/23/20
                    getline (NecOut, line); //read names
        ...
        while (NecOut.good() ) {
            ...
            for (int i=0; i<freq_step; i++) {
                ...
                if ( line.substr(0, line.find_first_of(":")) == "freq ") {
                    ...
                    //                    cout<<"freq["<<i<<"] = "<<Freq[i]<<" MHz"<<endl;
                    getline (NecOut, line); //read SWR
            	     Transm[i] = atof( line.substr(5, 11).c_str() ); //What says "SWR" in "Hpol_original_150mmHole_Ice_ARASim.txt" is actually transmission coefficient - MYL 01/23/20
                    getline (NecOut, line); //read names
